...
/**
 * @title  Collect Hot Essence Middleware
 * @author Nicky Harpor
 * @notice Discount applies on the essence after a target block has passed
 */
contract CollectHotEssenceMw is IEssenceMiddleware, FeeMw {
    ...
    mapping(uint256 => mapping(uint256 => CollectHotEssenceData))
     ...
     * currency is valid and approved. Also checks if discount is in range and target block has not passed.
     */
    function setEssenceMwData(
        ...
        require(discount <= Constants._MAX_BPS, "INVALID_DISCOUNT");
        require(targetBlock > block.number, "TARGET_BLOCK_IN_THE_PAST");
        ...
        _paidEssenceData[profileId][essenceId].targetBlock = targetBlock;
        _paidEssenceData[profileId][essenceId].discount = discount;
        _paidEssenceData[profileId][essenceId]
        .subscribeRequired = subscribeRequired;

        emit CollectHotEssenceMwSet(
            msg.sender,
            profileId,
            essenceId,
            totalSupply,
     ...
     * from the essence collector to the essence owner. Also applies discount if target block has passed.
     */
    function preProcess(
        uint256 profileId,
        uint256 essenceId,
        ...
        uint256 targetBlock = _paidEssenceData[profileId][essenceId].targetBlock;
        uint256 amount = _paidEssenceData[profileId][essenceId].amount;
        uint256 discount = _paidEssenceData[profileId][essenceId].discount;

        if (discount > 0) {
            if (targetBlock <= block.number) {
                amount = amount - ((amount * discount) / Constants._MAX_BPS);
            }
        }
